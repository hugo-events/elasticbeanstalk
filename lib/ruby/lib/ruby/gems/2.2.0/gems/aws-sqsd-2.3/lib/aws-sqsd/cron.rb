...
class AWS::EB::SQSD::Cron
    ...
    def initialize(config:, queue_url:)
        @config = config
        @queue_url = queue_url
    ...
    def send_message(dispatch_job)
        begin
            ...
            sqs.send_message({
              :queue_url => @queue_url,
              :message_body => 'elasticbeanstalk scheduled job',
              :message_attributes => {
                'beanstalk.sqsd.path' => {
                  :data_type => "String",
                  :string_value => dispatch_job.job.path
                },
                'beanstalk.sqsd.task_name' => {
                  :data_type => "String",
                  :string_value => dispatch_job.name
                },
                'beanstalk.sqsd.scheduled_time' => {
                  :data_type => "String",
                  :string_value => dispatch_job.next.to_s
                }
              }
            })
